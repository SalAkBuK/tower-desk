...
const mapAssigneeBuildingAccess = (value: any) => ({
  ...
  roleTemplateKey:
    asString(
      value?.roleTemplateKey ?? value?.roleKey ?? value?.key ?? value?.role,
    ) ?? "",
  roleTemplateName: asString(
    value?.roleTemplateName ?? value?.roleName ?? value?.name,
  ),
  scopeType:
    String(value?.scopeType ?? "BUILDING").toUpperCase() === "ORG"
      ? ("ORG" as const)
      : ("BUILDING" as const),
...
const mapRequestAssignee = (value: any): RequestAssignee | null => {
  ...
  return {
    ...
    name: asString(
      value?.name ?? value?.fullName ?? user?.name ?? user?.fullName,
    ),
    avatarUrl: asString(
      value?.avatarUrl ?? value?.avatar ?? user?.avatarUrl ?? user?.avatar,
    ),
    phone: asString(
      value?.phone ?? value?.phoneNumber ?? user?.phone ?? user?.phoneNumber,
    ),
    isActive:
...
const mapProviderMembership = (
  value: any,
): ServiceProviderMembership | null => {
...
const mapRequestOwnerApproval = (value: any) => {
  ...
  return {
    ...
    estimatedAmount:
      value?.estimatedAmount != null ? String(value.estimatedAmount) : null,
...
const mapServiceRequest = (
  ...
  return {
    ...
    availableWorkers: getArray(
      ...
      .filter(
        (entry) =>
          entry.membershipIsActive !== false && entry.userIsActive !== false,
      ),
    estimate: mapRequestEstimate(requestData.estimate),
    ownerApprovalStatus: mapOwnerApprovalStatus(
      requestData.ownerApprovalStatus ?? requestData.ownerApproval?.status,
    ),
...
const fetchPagedRequests = async (
  ...
  const page = await fetchAllPaged(async (cursor) => {
    const res = await fetchJson(
      `/org/buildings/${buildingId}/requests${buildRequestListSuffix({ ...filters, cursor })}`,
    );
...
const filterMockRequests = (
  ...
  requests.filter((request) => {
    if (filters?.status && mapRequestStatus(filters.status) !== request.status)
      return false;
    if (
      filters?.ownerApprovalStatus &&
      (request.ownerApprovalStatus ?? "NOT_REQUIRED") !==
        filters.ownerApprovalStatus
    )
      return false;
...
export async function getRequests(
  ...
  if (!USE_MOCK) {
    try {
      const role =
        useAuthStore.getState().user?.baseRole ??
      ...
      return data.map((entry: any) => {
        const requestData =
          entry?.request ?? entry?.item ?? entry?.data ?? entry;
...
export async function getRequestsForBuildings(
  ...
  await delay(800);
  return filterMockRequests(
    mockData.requests.filter((request) =>
      buildingIds.includes(request.buildingId),
    ),
    filters,
  );
}

export async function getRequestAssignees(
  buildingId: string,
): Promise<RequestAssignee[]> {
  if (!buildingId) return [];
  if (!USE_MOCK) {
    try {
      const res = await fetchJson(
        `/org/buildings/${buildingId}/requests/assignees`,
      );
  ...
  return mockData.users
    .filter(
      (user) =>
        (user.baseRole ?? user.role) === "employee" &&
...
export async function getRequest(
  ...
  if (!USE_MOCK) {
    try {
      ...
      if (buildingId) {
        try {
          commentsPayload = await fetchJson(
            `/org/buildings/${buildingId}/requests/${id}/comments`,
          );
        } catch {}
      }
      ...
      const commentsFromDetail: RequestComment[] = Array.isArray(data.comments)
        ...
        : [];
      const commentsFromEndpoint: RequestComment[] =
        getArray(commentsPayload).map(mapRequestComment);
      const commentsMap = new Map<string, RequestComment>();
      commentsFromDetail.forEach((comment) =>
        commentsMap.set(comment.id, comment),
      );
      commentsFromEndpoint.forEach((comment) =>
        commentsMap.set(comment.id, comment),
      );
...
export async function assignRequestProvider(
  requestId: string,
  ...
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/assign-provider`,
      {
        method: "POST",
  ...
  request.serviceProvider = {
    ...
    name: `Provider ${serviceProviderId}`,
  };
  request.serviceProviderAssignedTo = null;
  request.status = "assigned";
  request.queue = "ASSIGNED";
...
export async function assignRequestProviderWorker(
  requestId: string,
  userId: string,
  buildingId: string,
): Promise<ServiceRequest> {
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/assign-provider-worker`,
      {
        method: "POST",
...
export async function unassignRequestProvider(
  requestId: string,
  buildingId: string,
): Promise<ServiceRequest> {
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/unassign-provider`,
      { method: "POST" },
    );
    const updated = await getRequest(requestId, buildingId);
    if (!updated) throw new Error("Request not found");
...
export async function triageRequestPolicy(
  ...
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/policy-triage`,
      {
        method: "POST",
  ...
  const needsApproval =
    ...
    (payload.estimatedAmount ?? 0) > 1000;
  const needsEstimate =
    !payload.isEmergency &&
    !payload.isLikeForLike &&
    payload.estimatedAmount == null;

  request.ownerApproval = {
    ...request.ownerApproval,
    estimatedAmount:
      payload.estimatedAmount != null ? String(payload.estimatedAmount) : null,
...
export async function requestEstimate(
  ...
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/request-estimate`,
      {
        method: "POST",
        body: JSON.stringify({ serviceProviderId }),
      },
...
export async function submitRequestEstimate(
  ...
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/estimate`,
      {
        method: "POST",
        body: JSON.stringify(
          omitUndefined({
            ...
            ownerApprovalDeadlineAt:
              payload.ownerApprovalDeadlineAt ?? undefined,
          }),
        ),
      },
    );
  ...
  request.ownerApproval = {
    ...
    requestedByUserId: needsApproval
  ...
  request.policy = {
    ...
    summary: needsApproval
      ? "Estimate requires owner approval."
      : "Estimate approved for assignment.",
    isEmergency:
      payload.isEmergency ??
    ...
    isLikeForLike:
      payload.isLikeForLike ??
      request.policy?.isLikeForLike ??
      request.isLikeForLike ??
      null,
    isUpgrade:
      payload.isUpgrade ??
      request.policy?.isUpgrade ??
      request.isUpgrade ??
      null,
    isMajorReplacement:
      payload.isMajorReplacement ??
      request.policy?.isMajorReplacement ??
      request.isMajorReplacement ??
      null,
    isResponsibilityDisputed:
      payload.isResponsibilityDisputed ??
      request.policy?.isResponsibilityDisputed ??
...
export async function requestOwnerApprovalNow(
  ...
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/owner-approval/request-now`,
      {
        method: "POST",
        body: payload
          ? JSON.stringify(
              omitUndefined({
                approvalRequiredReason:
                  payload.approvalRequiredReason ?? undefined,
  ...
  request.ownerApproval = {
    ...
    estimatedAmount:
      payload?.estimatedAmount != null
        ? String(payload.estimatedAmount)
        : (request.ownerApproval?.estimatedAmount ?? null),
    estimatedCurrency:
      payload?.estimatedCurrency ??
  ...
  request.policy = {
    ...
    summary:
      request.policy?.summary ?? "Owner approval required before execution.",
    isEmergency:
      payload?.isEmergency ??
      request.policy?.isEmergency ??
      request.isEmergency ??
...
export async function sendOwnerApprovalReminder(
  requestId: string,
  buildingId: string,
): Promise<ServiceRequest> {
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/owner-approval/resend`,
      {
        method: "POST",
      },
    );
...
export async function overrideOwnerApproval(
  ...
  if (!USE_MOCK) {
    await fetchJson(
      `/org/buildings/${buildingId}/requests/${requestId}/owner-approval/override`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
